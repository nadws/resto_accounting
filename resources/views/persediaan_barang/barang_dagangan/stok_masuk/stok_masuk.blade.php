<x-theme.app title="{{ $title }}" nav="Y" rot1="barang_dagangan.index" rot2="barang_dagangan.stok_masuk"
    ...
    <x-slot name="cardHeader">
        ...
        <div class="row justify-content-end">
            ...
            <div class="col-lg-3">
                @if (!empty($create))
                    <a href="{{ route('barang_dagangan.add') }}" class="btn btn-primary float-end"> <i
                            class="fas fa-plus"></i> Tambah</a>
                @endif

                <x-theme.akses :halaman="$halaman" route="barang_dagangan.stok_masuk" />
            </div>
        </div>
    </x-slot>
                                            ...
                                            @if (!empty($edit))
                                                <li>
                                                    <a class="dropdown-item text-primary edit"
                                                        href="{{ route('barang_dagangan.add', ['no_nota' => encrypt($d->no_nota)]) }}"><i
        ...
        <script>
            $(document).ready(function() {
                ...
                $(".select-gudang").change(function(e) {
                    ...
                    document.location.href = `/barang_dagangan/stok_masuk/${gudang_id}`
                });
