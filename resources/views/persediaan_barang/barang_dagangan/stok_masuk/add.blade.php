<x-theme.app title="{{ $title }}" table="Y" sizeCard="12">
    ...
    <x-slot name="cardBody">
        <form action="{{ route('barang_dagangan.store') }}" method="post" id="save_stok_masuk">
            @csrf
    ...
    <x-slot name="cardFooter">
        ...
        <a href="{{ route('barang_dagangan.index') }}" class="float-end btn btn-outline-primary me-2">Batal</a>
        </form>
    ...
    @section('scripts')
        <script>
            ...
            function load_menu() {
                $.ajax({
                    type: "GET",
                    url: "{{ route('barang_dagangan.load_menu') }}?no_nota={{ $no_notas ?? '' }}",
                    data: {
                        'no_nota': "{{ $no_notas ?? '' }}",
                    },
                    success: function(r) {
