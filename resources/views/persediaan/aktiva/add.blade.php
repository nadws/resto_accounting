<x-theme.app title="{{ $title }}" table="Y" sizeCard="12">
    ...
    <x-slot name="cardBody">
        <form action="{{ route('aktiva.save_aktiva') }}" method="post" class="save_jurnal">
            @csrf
            <section class="row">
                <div class="col-lg-12">
    ...
    <x-slot name="cardFooter">
        ...
        <a href="{{ route('aktiva.index') }}" class="float-end btn btn-outline-primary me-2">Batal</a>
        </form>
    </x-slot>
    @section('scripts')
        <script>
            $(document).ready(function() {
                ...
                $(document).on("click", ".tbh_baris_aktiva", function() {
                    count = count + 1;
                    $.ajax({
                        url: "/aktiva/tambah_baris_aktiva?count=" + count,
                        type: "Get",
                ...
                $(document).on("change", ".pilih_kelompok", function() {
                    ...
                    $.ajax({
                        type: "GET",
                        url: "/aktiva/get_data_kelompok?id_kelompok=" + id_kelompok,
                        dataType: "json",
                        success: function(data) {
