<x-theme.app title="{{ $title }}" table="Y" sizeCard="12">
    ...
    <x-slot name="cardBody">
        ...
        <form action="{{ route('piutang.create') }}" method="post" class="save_jurnal">
            @csrf
            ...
            <section class="row">
                <table class="table table-striped">
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td>
                                <input type="text" class="form-control" value="PBYRAGL-{{ $no_pembayaran }}" readonly>
                                <input type="hidden" class="form-control" name="no_pembayaran"
                                    value="PBYRAGL-{{ $no_pembayaran }}" readonly>
                            </td>
                        </tr>
                    </tbody>
                </table>
                ...
                <div class="col-lg-12">
                    <table class="table table-striped">
                        <thead>
                            <tr>
                                <th class="dhead" width="5%">Nota Manual</th>
                        ...
                        <tbody>
                            ...
                            @foreach ($no_order as $no => $p)
                                @php
                                    $getRow = DB::selectOne("SELECT a.* FROM invoice_agl as a
                                        WHERE a.no_nota = '$p'
                                        ORDER BY a.id_invoice_bk ASC");
                                    
                                    $getRowKredit = DB::selectOne("SELECT a.no_nota, a.no_penjualan, a.status,(a.total_rp - c.kredit) as total_rp,a.tgl, c.kredit, c.debit FROM `invoice_agl` as a
                                    LEFT JOIN (
                    ...
                    <div class="col-lg-6 float-end">
                        ...
                        <table class="" width="100%">
                            <tr>
                                ...
                                <input type="hidden" class="total_hutangTetap fs-5" value="{{ $ttlKredit }}">
                                <td width="40%" class="total fs-5" style="text-align: right;">
                                    Rp.{{ number_format($ttlKredit, 2) }}
                                </td>
                                <td width="40%" class="total_kredit fs-5" style="text-align: right;">
                                    Rp. {{ number_format($ttlKredit, 2) }}
                                </td>
                            </tr>
                            <tr>
                                <td class="cselisih fs-6" colspan="2">Sisa Hutang</td>
                                <td style="text-align: right;" class="selisih cselisih fs-6">Rp.
                                    0</td>
                            </tr>
                        </table>
                    </div>
    ...
    <x-slot name="cardFooter">
        ...
        <a href="{{ route('piutang.index') }}" class="float-end btn btn-outline-primary me-2">Batal</a>
        </form>
    </x-slot>
    ...
    @section('scripts')
        <script>
            ...
            convertRpKoma('rp-nohide', 'rp-hide')
            convertRpKoma('setor-nohide', 'setor-hide')
            convertRpKoma('kredit-nohide', 'kredit-hide')
        </script>
    @endsection
</x-theme.app>
